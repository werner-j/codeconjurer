...
public class CodeConjurer {
	...
	public void setSearchFinished(Search search) {
		...
		// if (searches.get(editor).size() > 1) {
		// logger.debug("Remove " + search + " for editor "
		// + search.getEditor().getEditorInput());
		// searches.get(editor).remove(search);
		// if (searches.get(editor).isEmpty()) {
		// searches.remove(editor);
		// }
		// } else {
		// logger.debug(search + " stored for editor "
		// + search.getEditor().getEditorInput());
		// }
	}
